...
use super::{HasInputs, HasOutputs};
...
pub trait Node: HasInputs + HasOutputs + Send + Sync {
    ...
    fn handle_event_for_conn(&mut self, conn_name: &String, ev: Arc<Event>) -> TaskResult;
    ...
    fn handle_event(&mut self, conn_name: &String, ev: Arc<Event>) -> TaskResult {
        match self.handle_event_for_conn(conn_name, ev.clone()) {
            Ok((events, status)) => {
