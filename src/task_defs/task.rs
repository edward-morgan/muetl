...
impl Input<String> for Test {
    ...
    fn handle(&mut self, ctx: &MuetlContext, input: &String) -> TaskResult {
        Ok((vec![], None))
    }
}
...
impl Input<u32> for Test {
    ...
    fn handle(&mut self, ctx: &MuetlContext, input: &u32) -> TaskResult {
        Ok((vec![], None))
    }
}
...
impl node::Node for Test {
    ...
    fn handle_event_for_conn(
        ...
        match conn_name.as_str() {
            "user id" => self.handle_input_1(ctx, ev.get_data()),
            _ => Err(format!(
...
impl Test {
    ...
    fn handle_input_1(
        ...
        if data.type_id() == TypeId::of::<String>() {
            <Self as Input<String>>::handle(self, ctx, data.downcast_ref::<String>().unwrap())
        } else if data.type_id() == TypeId::of::<u32>() {
            <Self as Input<u32>>::handle(self, ctx, data.downcast_ref::<u32>().unwrap())
        } else {
            Err(format!(
